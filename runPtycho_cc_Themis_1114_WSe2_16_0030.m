...
% Last modified by Chia-Hao Lee@UIUC, 2022/11/15
...
cSAXS_matlab_path = '/home/chiahao3/fold_slice';
ptycho_matlab_path = '/home/chiahao3/fold_slice/ptycho';
...
exp_dir = '/scratch/users/chiahao3/20221114_TEM_ThemisZ_80kV_WSe2_16_EMPAD/'; %change this for different experiment
scan_dir = '0030_18mrad_cl185mm_20.5Mx_df0nm_10pA_step128'; % change this for different scans
...
voltage = 80;
alpha0 = 18.44; %convergence semi angle # 18->18.44, 25.2->24.92, 36->36.4, 54->55.45
...
Np_p = [128,128]; % size of diffraction patterns / probe used during reconstruction. can crop/pad to 64/256
...
initial_probe_file = strcat(base_path, sprintf('%i/init_probe_%i.mat', scan_number, Np_p(1)));
%init_recon_file = '/scratch/users/chiahao3/20220712_TEM_Talos_WSe2_16_BL_EMPAD/0032_WSe2_bi_10.5mrad_cl_205_28.5mx_def0_1/1/roi1_Ndp128_step128/MLs_L1_p10_g128_pc0_noModel_updW100_vp2_vi_mm_dpFlip_T/Niter2800.mat';
...
fin = fread(fopen(strcat(base_path, scan_name),'r'), dim_x * dim_y * N_scan_x * N_scan_y,'float32'); % Load as 1-D vector
...
%% Step 3: Pre-process the size of the diffraction patterns (pad, crop)
[ndpx, ndpy, npx, npy] = size(cbed); % crop / pad the diffraction patterns dp
%cbed = cbed - min(min(min(min(cbed)))); % quick background subtraction, added by Chia-Hao
...
mkdir(save_dir)
roi_label = sprintf('%i_Ndp%i_step%i', scan_number, Np_p(1), Npx);
...
% save initial probe
save(strcat(save_dir,sprintf('/init_probe_%i.mat', Np_p(1))),'probe','p')
...
eng.custom_data_flip = [0,1,1];         % apply custom flip of the data [fliplr, flipud, transpose]  - can be used for quick testing of reconstruction with various flips or for reflection ptychography 
